.data
label_0_str:
.asciiz " "
.align 2
label_1_str:
.asciiz "\n"
.align 2
label_2_str:
.asciiz "Enter the number of rows for Pascal\'s Triangle: "
.align 2
# Allocated labels for virtual registers
label_112_v33:
.space 4
label_311_v122:
.space 4
label_262_v101:
.space 4
label_123_v36:
.space 4
label_203_v72:
.space 4
label_155_v53:
.space 4
label_295_v114:
.space 4
label_148_v45:
.space 4
label_191_v57:
.space 4
label_215_v76:
.space 4
label_182_v65:
.space 4
label_200_v71:
.space 4
label_141_v48:
.space 4
label_245_v92:
.space 4
label_256_v98:
.space 4
label_294_v113:
.space 4
label_317_v123:
.space 4
label_145_v50:
.space 4
label_144_v49:
.space 4
label_206_v74:
.space 4
label_337_v134:
.space 4
label_214_v78:
.space 4
label_225_v82:
.space 4
label_92_v27:
.space 4
label_57_v11:
.space 4
label_193_v56:
.space 4
label_116_v37:
.space 4
label_176_v64:
.space 4
label_345_v138:
.space 4
label_109_v25:
.space 4
label_229_v80:
.space 4
label_154_v52:
.space 4
label_38_v0:
.space 4
label_106_v32:
.space 4
label_186_v67:
.space 4
label_172_v62:
.space 4
label_66_v15:
.space 4
label_137_v46:
.space 4
label_196_v69:
.space 4
label_189_v58:
.space 4
label_243_v90:
.space 4
label_159_v55:
.space 4
label_197_v34:
.space 4
label_308_v121:
.space 4
label_289_v105:
.space 4
label_302_v118:
.space 4
label_185_v66:
.space 4
label_96_v29:
.space 4
label_286_v112:
.space 4
label_204_v73:
.space 4
label_265_v100:
.space 4
label_46_v5:
.space 4
label_250_v93:
.space 4
label_69_v16:
.space 4
label_130_v42:
.space 4
label_251_v95:
.space 4
label_79_v21:
.space 4
label_168_v60:
.space 4
label_253_v96:
.space 4
label_299_v116:
.space 4
label_52_v7:
.space 4
label_275_v108:
.space 4
label_279_v106:
.space 4
label_257_v97:
.space 4
label_70_v17:
.space 4
label_242_v91:
.space 4
label_164_v43:
.space 4
label_40_v2:
.space 4
label_89_v14:
.space 4
label_158_v54:
.space 4
label_335_v129:
.space 4
label_269_v103:
.space 4
label_105_v31:
.space 4
label_264_v102:
.space 4
label_285_v111:
.space 4
label_120_v39:
.space 4
label_236_v86:
.space 4
label_119_v38:
.space 4
label_74_v18:
.space 4
label_325_v128:
.space 4
label_194_v68:
.space 4
label_222_v81:
.space 4
label_43_v4:
.space 4
label_102_v30:
.space 4
label_99_v26:
.space 4
label_331_v131:
.space 4
label_140_v47:
.space 4
label_332_v132:
.space 4
label_239_v89:
.space 4
label_228_v84:
.space 4
label_48_v6:
.space 4
label_298_v115:
.space 4
label_45_v3:
.space 4
label_306_v120:
.space 4
label_321_v126:
.space 4
label_171_v61:
.space 4
label_282_v110:
.space 4
label_328_v130:
.space 4
label_231_v85:
.space 4
label_309_v117:
.space 4
label_64_v13:
.space 4
label_37_v1:
.space 4
label_324_v127:
.space 4
label_314_v124:
.space 4
label_237_v88:
.space 4
label_60_v10:
.space 4
label_259_v99:
.space 4
label_340_v136:
.space 4
label_272_v107:
.space 4
label_175_v63:
.space 4
label_162_v44:
.space 4
label_226_v83:
.space 4
label_291_v104:
.space 4
label_51_v8:
.space 4
label_82_v22:
.space 4
label_212_v77:
.space 4
label_305_v119:
.space 4
label_219_v79:
.space 4
label_95_v28:
.space 4
label_126_v40:
.space 4
label_179_v59:
.space 4
label_59_v12:
.space 4
label_207_v70:
.space 4
label_342_v135:
.space 4
label_334_v133:
.space 4
label_209_v75:
.space 4
label_343_v137:
.space 4
label_151_v51:
.space 4
label_81_v20:
.space 4
label_129_v41:
.space 4
label_86_v23:
.space 4
label_54_v9:
.space 4
label_133_v35:
.space 4
label_248_v94:
.space 4
label_76_v19:
.space 4
label_234_v87:
.space 4
label_113_v24:
.space 4
label_316_v125:
.space 4
label_276_v109:
.space 4
# Allocated labels for virtual registers
label_354_v143:
.space 4
label_355_v141:
.space 4
label_352_v142:
.space 4
label_357_v144:
.space 4
label_349_v140:
.space 4
label_360_v145:
.space 4
label_361_v146:
.space 4
label_348_v139:
.space 4
# Allocated labels for virtual registers
# Allocated labels for virtual registers
# Allocated labels for virtual registers
# Allocated labels for virtual registers
# Allocated labels for virtual registers
# Allocated labels for virtual registers

.text
print_pascals_triangle:
# Original instruction: addiu $sp,$sp,-4
addiu $sp,$sp,-4
# Original instruction: sw $fp,0($sp)
sw $fp,0($sp)
# Original instruction: addi $fp,$sp,0
addi $fp,$sp,0
# Original instruction: addiu $sp,$sp,-4
addiu $sp,$sp,-4
# Original instruction: sw $ra,0($sp)
sw $ra,0($sp)
# Original instruction: addiu $sp,$sp,-10008
addiu $sp,$sp,-10008
# Original instruction: pushRegisters
la $t0,label_112_v33
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_311_v122
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_262_v101
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_123_v36
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_203_v72
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_155_v53
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_295_v114
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_148_v45
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_191_v57
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_215_v76
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_182_v65
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_200_v71
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_141_v48
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_245_v92
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_256_v98
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_294_v113
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_317_v123
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_145_v50
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_144_v49
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_206_v74
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_337_v134
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_214_v78
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_225_v82
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_92_v27
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_57_v11
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_193_v56
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_116_v37
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_176_v64
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_345_v138
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_109_v25
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_229_v80
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_154_v52
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_38_v0
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_106_v32
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_186_v67
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_172_v62
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_66_v15
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_137_v46
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_196_v69
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_189_v58
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_243_v90
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_159_v55
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_197_v34
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_308_v121
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_289_v105
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_302_v118
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_185_v66
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_96_v29
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_286_v112
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_204_v73
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_265_v100
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_46_v5
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_250_v93
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_69_v16
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_130_v42
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_251_v95
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_79_v21
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_168_v60
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_253_v96
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_299_v116
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_52_v7
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_275_v108
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_279_v106
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_257_v97
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_70_v17
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_242_v91
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_164_v43
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_40_v2
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_89_v14
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_158_v54
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_335_v129
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_269_v103
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_105_v31
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_264_v102
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_285_v111
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_120_v39
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_236_v86
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_119_v38
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_74_v18
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_325_v128
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_194_v68
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_222_v81
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_43_v4
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_102_v30
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_99_v26
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_331_v131
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_140_v47
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_332_v132
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_239_v89
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_228_v84
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_48_v6
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_298_v115
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_45_v3
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_306_v120
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_321_v126
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_171_v61
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_282_v110
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_328_v130
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_231_v85
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_309_v117
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_64_v13
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_37_v1
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_324_v127
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_314_v124
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_237_v88
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_60_v10
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_259_v99
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_340_v136
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_272_v107
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_175_v63
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_162_v44
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_226_v83
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_291_v104
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_51_v8
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_82_v22
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_212_v77
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_305_v119
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_219_v79
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_95_v28
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_126_v40
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_179_v59
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_59_v12
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_207_v70
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_342_v135
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_334_v133
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_209_v75
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_343_v137
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_151_v51
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_81_v20
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_129_v41
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_86_v23
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_54_v9
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_133_v35
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_248_v94
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_76_v19
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_234_v87
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_113_v24
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_316_v125
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
la $t0,label_276_v109
lw $t0,0($t0)
addiu $sp,$sp,-4
sw $t0,0($sp)
# Original instruction: addiu v0,$fp,-10008
addiu $t5,$fp,-10008
la $t0,label_38_v0
sw $t5,0($t0)
# Original instruction: li v1,0
li $t5,0
la $t0,label_37_v1
sw $t5,0($t0)
# Original instruction: sw v1,0(v0)
la $t5,label_37_v1
lw $t5,0($t5)
la $t4,label_38_v0
lw $t4,0($t4)
sw $t5,0($t4)
# Original instruction: addiu v2,$fp,-10008
addiu $t5,$fp,-10008
la $t0,label_40_v2
sw $t5,0($t0)
# Original instruction: lw v3,0(v2)
la $t5,label_40_v2
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_45_v3
sw $t4,0($t0)
# Original instruction: addiu v4,$fp,4
addiu $t5,$fp,4
la $t0,label_43_v4
sw $t5,0($t0)
# Original instruction: lw v5,0(v4)
la $t5,label_43_v4
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_46_v5
sw $t4,0($t0)
# Original instruction: slt v6,v3,v5
la $t5,label_45_v3
lw $t5,0($t5)
la $t4,label_46_v5
lw $t4,0($t4)
slt $t3,$t5,$t4
la $t0,label_48_v6
sw $t3,0($t0)
# Original instruction: beqz v6,label_4_next
la $t5,label_48_v6
lw $t5,0($t5)
beqz $t5,label_4_next
label_6_body:
# Original instruction: addiu v7,$fp,-10012
addiu $t5,$fp,-10012
la $t0,label_52_v7
sw $t5,0($t0)
# Original instruction: li v8,0
li $t5,0
la $t0,label_51_v8
sw $t5,0($t0)
# Original instruction: sw v8,0(v7)
la $t5,label_51_v8
lw $t5,0($t5)
la $t4,label_52_v7
lw $t4,0($t4)
sw $t5,0($t4)
# Original instruction: addiu v9,$fp,-10012
addiu $t5,$fp,-10012
la $t0,label_54_v9
sw $t5,0($t0)
# Original instruction: lw v10,0(v9)
la $t5,label_54_v9
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_60_v10
sw $t4,0($t0)
# Original instruction: addiu v11,$fp,-10008
addiu $t5,$fp,-10008
la $t0,label_57_v11
sw $t5,0($t0)
# Original instruction: lw v12,0(v11)
la $t5,label_57_v11
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_59_v12
sw $t4,0($t0)
# Original instruction: slt v13,v12,v10
la $t5,label_59_v12
lw $t5,0($t5)
la $t4,label_60_v10
lw $t4,0($t4)
slt $t3,$t5,$t4
la $t0,label_64_v13
sw $t3,0($t0)
# Original instruction: xori v13,v13,1
la $t4,label_64_v13
lw $t4,0($t4)
xori $t4,$t4,1
la $t0,label_64_v13
sw $t4,0($t0)
# Original instruction: beqz v13,label_7_next
la $t5,label_64_v13
lw $t5,0($t5)
beqz $t5,label_7_next
label_9_body:
# Original instruction: addiu v15,$fp,-10012
addiu $t5,$fp,-10012
la $t0,label_66_v15
sw $t5,0($t0)
# Original instruction: lw v16,0(v15)
la $t5,label_66_v15
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_69_v16
sw $t4,0($t0)
# Original instruction: li v17,0
li $t5,0
la $t0,label_70_v17
sw $t5,0($t0)
# Original instruction: xor v18,v16,v17
la $t5,label_69_v16
lw $t5,0($t5)
la $t4,label_70_v17
lw $t4,0($t4)
xor $t3,$t5,$t4
la $t0,label_74_v18
sw $t3,0($t0)
# Original instruction: sltiu v18,v18,1
la $t4,label_74_v18
lw $t4,0($t4)
sltiu $t4,$t4,1
la $t0,label_74_v18
sw $t4,0($t0)
# Original instruction: bne v18,$zero,label_12_
la $t5,label_74_v18
lw $t5,0($t5)
bne $t5,$zero,label_12_
# Original instruction: addiu v19,$fp,-10012
addiu $t5,$fp,-10012
la $t0,label_76_v19
sw $t5,0($t0)
# Original instruction: lw v20,0(v19)
la $t5,label_76_v19
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_81_v20
sw $t4,0($t0)
# Original instruction: addiu v21,$fp,-10008
addiu $t5,$fp,-10008
la $t0,label_79_v21
sw $t5,0($t0)
# Original instruction: lw v22,0(v21)
la $t5,label_79_v21
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_82_v22
sw $t4,0($t0)
# Original instruction: xor v23,v20,v22
la $t5,label_81_v20
lw $t5,0($t5)
la $t4,label_82_v22
lw $t4,0($t4)
xor $t3,$t5,$t4
la $t0,label_86_v23
sw $t3,0($t0)
# Original instruction: sltiu v23,v23,1
la $t4,label_86_v23
lw $t4,0($t4)
sltiu $t4,$t4,1
la $t0,label_86_v23
sw $t4,0($t0)
# Original instruction: bne v23,$zero,label_12_
la $t5,label_86_v23
lw $t5,0($t5)
bne $t5,$zero,label_12_
# Original instruction: li v14,0
li $t5,0
la $t0,label_89_v14
sw $t5,0($t0)
# Original instruction: j label_13_
j label_13_
label_12_:
# Original instruction: li v14,1
li $t5,1
la $t0,label_89_v14
sw $t5,0($t0)
label_13_:
# Original instruction: beqz v14,label_10_ELSE
la $t5,label_89_v14
lw $t5,0($t5)
beqz $t5,label_10_ELSE
# Original instruction: addiu v26,$fp,-10004
addiu $t5,$fp,-10004
la $t0,label_99_v26
sw $t5,0($t0)
# Original instruction: addiu v27,$fp,-10008
addiu $t5,$fp,-10008
la $t0,label_92_v27
sw $t5,0($t0)
# Original instruction: lw v28,0(v27)
la $t5,label_92_v27
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_95_v28
sw $t4,0($t0)
# Original instruction: li v29,200
li $t5,200
la $t0,label_96_v29
sw $t5,0($t0)
# Original instruction: mul v25,v28,v29
la $t5,label_95_v28
lw $t5,0($t5)
la $t4,label_96_v29
lw $t4,0($t4)
mul $t3,$t5,$t4
la $t0,label_109_v25
sw $t3,0($t0)
# Original instruction: add v25,v25,v26
la $t3,label_109_v25
lw $t3,0($t3)
la $t4,label_99_v26
lw $t4,0($t4)
add $t3,$t3,$t4
la $t0,label_109_v25
sw $t3,0($t0)
# Original instruction: addiu v30,$fp,-10012
addiu $t5,$fp,-10012
la $t0,label_102_v30
sw $t5,0($t0)
# Original instruction: lw v31,0(v30)
la $t5,label_102_v30
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_105_v31
sw $t4,0($t0)
# Original instruction: li v32,4
li $t5,4
la $t0,label_106_v32
sw $t5,0($t0)
# Original instruction: mul v24,v31,v32
la $t5,label_105_v31
lw $t5,0($t5)
la $t4,label_106_v32
lw $t4,0($t4)
mul $t3,$t5,$t4
la $t0,label_113_v24
sw $t3,0($t0)
# Original instruction: add v24,v24,v25
la $t3,label_113_v24
lw $t3,0($t3)
la $t4,label_109_v25
lw $t4,0($t4)
add $t3,$t3,$t4
la $t0,label_113_v24
sw $t3,0($t0)
# Original instruction: li v33,1
li $t5,1
la $t0,label_112_v33
sw $t5,0($t0)
# Original instruction: sw v33,0(v24)
la $t5,label_112_v33
lw $t5,0($t5)
la $t4,label_113_v24
lw $t4,0($t4)
sw $t5,0($t4)
# Original instruction: j label_11_END
j label_11_END
label_10_ELSE:
# Original instruction: addiu v36,$fp,-10004
addiu $t5,$fp,-10004
la $t0,label_123_v36
sw $t5,0($t0)
# Original instruction: addiu v37,$fp,-10008
addiu $t5,$fp,-10008
la $t0,label_116_v37
sw $t5,0($t0)
# Original instruction: lw v38,0(v37)
la $t5,label_116_v37
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_119_v38
sw $t4,0($t0)
# Original instruction: li v39,200
li $t5,200
la $t0,label_120_v39
sw $t5,0($t0)
# Original instruction: mul v35,v38,v39
la $t5,label_119_v38
lw $t5,0($t5)
la $t4,label_120_v39
lw $t4,0($t4)
mul $t3,$t5,$t4
la $t0,label_133_v35
sw $t3,0($t0)
# Original instruction: add v35,v35,v36
la $t3,label_133_v35
lw $t3,0($t3)
la $t4,label_123_v36
lw $t4,0($t4)
add $t3,$t3,$t4
la $t0,label_133_v35
sw $t3,0($t0)
# Original instruction: addiu v40,$fp,-10012
addiu $t5,$fp,-10012
la $t0,label_126_v40
sw $t5,0($t0)
# Original instruction: lw v41,0(v40)
la $t5,label_126_v40
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_129_v41
sw $t4,0($t0)
# Original instruction: li v42,4
li $t5,4
la $t0,label_130_v42
sw $t5,0($t0)
# Original instruction: mul v34,v41,v42
la $t5,label_129_v41
lw $t5,0($t5)
la $t4,label_130_v42
lw $t4,0($t4)
mul $t3,$t5,$t4
la $t0,label_197_v34
sw $t3,0($t0)
# Original instruction: add v34,v34,v35
la $t3,label_197_v34
lw $t3,0($t3)
la $t4,label_133_v35
lw $t4,0($t4)
add $t3,$t3,$t4
la $t0,label_197_v34
sw $t3,0($t0)
# Original instruction: addiu v45,$fp,-10004
addiu $t5,$fp,-10004
la $t0,label_148_v45
sw $t5,0($t0)
# Original instruction: addiu v46,$fp,-10008
addiu $t5,$fp,-10008
la $t0,label_137_v46
sw $t5,0($t0)
# Original instruction: lw v47,0(v46)
la $t5,label_137_v46
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_140_v47
sw $t4,0($t0)
# Original instruction: li v48,1
li $t5,1
la $t0,label_141_v48
sw $t5,0($t0)
# Original instruction: sub v49,v47,v48
la $t5,label_140_v47
lw $t5,0($t5)
la $t4,label_141_v48
lw $t4,0($t4)
sub $t3,$t5,$t4
la $t0,label_144_v49
sw $t3,0($t0)
# Original instruction: li v50,200
li $t5,200
la $t0,label_145_v50
sw $t5,0($t0)
# Original instruction: mul v44,v49,v50
la $t5,label_144_v49
lw $t5,0($t5)
la $t4,label_145_v50
lw $t4,0($t4)
mul $t3,$t5,$t4
la $t0,label_162_v44
sw $t3,0($t0)
# Original instruction: add v44,v44,v45
la $t3,label_162_v44
lw $t3,0($t3)
la $t4,label_148_v45
lw $t4,0($t4)
add $t3,$t3,$t4
la $t0,label_162_v44
sw $t3,0($t0)
# Original instruction: addiu v51,$fp,-10012
addiu $t5,$fp,-10012
la $t0,label_151_v51
sw $t5,0($t0)
# Original instruction: lw v52,0(v51)
la $t5,label_151_v51
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_154_v52
sw $t4,0($t0)
# Original instruction: li v53,1
li $t5,1
la $t0,label_155_v53
sw $t5,0($t0)
# Original instruction: sub v54,v52,v53
la $t5,label_154_v52
lw $t5,0($t5)
la $t4,label_155_v53
lw $t4,0($t4)
sub $t3,$t5,$t4
la $t0,label_158_v54
sw $t3,0($t0)
# Original instruction: li v55,4
li $t5,4
la $t0,label_159_v55
sw $t5,0($t0)
# Original instruction: mul v43,v54,v55
la $t5,label_158_v54
lw $t5,0($t5)
la $t4,label_159_v55
lw $t4,0($t4)
mul $t3,$t5,$t4
la $t0,label_164_v43
sw $t3,0($t0)
# Original instruction: add v43,v43,v44
la $t3,label_164_v43
lw $t3,0($t3)
la $t4,label_162_v44
lw $t4,0($t4)
add $t3,$t3,$t4
la $t0,label_164_v43
sw $t3,0($t0)
# Original instruction: lw v56,0(v43)
la $t5,label_164_v43
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_193_v56
sw $t4,0($t0)
# Original instruction: addiu v59,$fp,-10004
addiu $t5,$fp,-10004
la $t0,label_179_v59
sw $t5,0($t0)
# Original instruction: addiu v60,$fp,-10008
addiu $t5,$fp,-10008
la $t0,label_168_v60
sw $t5,0($t0)
# Original instruction: lw v61,0(v60)
la $t5,label_168_v60
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_171_v61
sw $t4,0($t0)
# Original instruction: li v62,1
li $t5,1
la $t0,label_172_v62
sw $t5,0($t0)
# Original instruction: sub v63,v61,v62
la $t5,label_171_v61
lw $t5,0($t5)
la $t4,label_172_v62
lw $t4,0($t4)
sub $t3,$t5,$t4
la $t0,label_175_v63
sw $t3,0($t0)
# Original instruction: li v64,200
li $t5,200
la $t0,label_176_v64
sw $t5,0($t0)
# Original instruction: mul v58,v63,v64
la $t5,label_175_v63
lw $t5,0($t5)
la $t4,label_176_v64
lw $t4,0($t4)
mul $t3,$t5,$t4
la $t0,label_189_v58
sw $t3,0($t0)
# Original instruction: add v58,v58,v59
la $t3,label_189_v58
lw $t3,0($t3)
la $t4,label_179_v59
lw $t4,0($t4)
add $t3,$t3,$t4
la $t0,label_189_v58
sw $t3,0($t0)
# Original instruction: addiu v65,$fp,-10012
addiu $t5,$fp,-10012
la $t0,label_182_v65
sw $t5,0($t0)
# Original instruction: lw v66,0(v65)
la $t5,label_182_v65
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_185_v66
sw $t4,0($t0)
# Original instruction: li v67,4
li $t5,4
la $t0,label_186_v67
sw $t5,0($t0)
# Original instruction: mul v57,v66,v67
la $t5,label_185_v66
lw $t5,0($t5)
la $t4,label_186_v67
lw $t4,0($t4)
mul $t3,$t5,$t4
la $t0,label_191_v57
sw $t3,0($t0)
# Original instruction: add v57,v57,v58
la $t3,label_191_v57
lw $t3,0($t3)
la $t4,label_189_v58
lw $t4,0($t4)
add $t3,$t3,$t4
la $t0,label_191_v57
sw $t3,0($t0)
# Original instruction: lw v68,0(v57)
la $t5,label_191_v57
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_194_v68
sw $t4,0($t0)
# Original instruction: add v69,v56,v68
la $t5,label_193_v56
lw $t5,0($t5)
la $t4,label_194_v68
lw $t4,0($t4)
add $t3,$t5,$t4
la $t0,label_196_v69
sw $t3,0($t0)
# Original instruction: sw v69,0(v34)
la $t5,label_196_v69
lw $t5,0($t5)
la $t4,label_197_v34
lw $t4,0($t4)
sw $t5,0($t4)
label_11_END:
# Original instruction: addiu v70,$fp,-10012
addiu $t5,$fp,-10012
la $t0,label_207_v70
sw $t5,0($t0)
# Original instruction: addiu v71,$fp,-10012
addiu $t5,$fp,-10012
la $t0,label_200_v71
sw $t5,0($t0)
# Original instruction: lw v72,0(v71)
la $t5,label_200_v71
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_203_v72
sw $t4,0($t0)
# Original instruction: li v73,1
li $t5,1
la $t0,label_204_v73
sw $t5,0($t0)
# Original instruction: add v74,v72,v73
la $t5,label_203_v72
lw $t5,0($t5)
la $t4,label_204_v73
lw $t4,0($t4)
add $t3,$t5,$t4
la $t0,label_206_v74
sw $t3,0($t0)
# Original instruction: sw v74,0(v70)
la $t5,label_206_v74
lw $t5,0($t5)
la $t4,label_207_v70
lw $t4,0($t4)
sw $t5,0($t4)
label_8_posttest:
# Original instruction: addiu v75,$fp,-10012
addiu $t5,$fp,-10012
la $t0,label_209_v75
sw $t5,0($t0)
# Original instruction: lw v76,0(v75)
la $t5,label_209_v75
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_215_v76
sw $t4,0($t0)
# Original instruction: addiu v77,$fp,-10008
addiu $t5,$fp,-10008
la $t0,label_212_v77
sw $t5,0($t0)
# Original instruction: lw v78,0(v77)
la $t5,label_212_v77
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_214_v78
sw $t4,0($t0)
# Original instruction: slt v79,v78,v76
la $t5,label_214_v78
lw $t5,0($t5)
la $t4,label_215_v76
lw $t4,0($t4)
slt $t3,$t5,$t4
la $t0,label_219_v79
sw $t3,0($t0)
# Original instruction: xori v79,v79,1
la $t4,label_219_v79
lw $t4,0($t4)
xori $t4,$t4,1
la $t0,label_219_v79
sw $t4,0($t0)
# Original instruction: bnez v79,label_9_body
la $t5,label_219_v79
lw $t5,0($t5)
bnez $t5,label_9_body
label_7_next:
# Original instruction: addiu v80,$fp,-10008
addiu $t5,$fp,-10008
la $t0,label_229_v80
sw $t5,0($t0)
# Original instruction: addiu v81,$fp,-10008
addiu $t5,$fp,-10008
la $t0,label_222_v81
sw $t5,0($t0)
# Original instruction: lw v82,0(v81)
la $t5,label_222_v81
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_225_v82
sw $t4,0($t0)
# Original instruction: li v83,1
li $t5,1
la $t0,label_226_v83
sw $t5,0($t0)
# Original instruction: add v84,v82,v83
la $t5,label_225_v82
lw $t5,0($t5)
la $t4,label_226_v83
lw $t4,0($t4)
add $t3,$t5,$t4
la $t0,label_228_v84
sw $t3,0($t0)
# Original instruction: sw v84,0(v80)
la $t5,label_228_v84
lw $t5,0($t5)
la $t4,label_229_v80
lw $t4,0($t4)
sw $t5,0($t4)
label_5_posttest:
# Original instruction: addiu v85,$fp,-10008
addiu $t5,$fp,-10008
la $t0,label_231_v85
sw $t5,0($t0)
# Original instruction: lw v86,0(v85)
la $t5,label_231_v85
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_236_v86
sw $t4,0($t0)
# Original instruction: addiu v87,$fp,4
addiu $t5,$fp,4
la $t0,label_234_v87
sw $t5,0($t0)
# Original instruction: lw v88,0(v87)
la $t5,label_234_v87
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_237_v88
sw $t4,0($t0)
# Original instruction: slt v89,v86,v88
la $t5,label_236_v86
lw $t5,0($t5)
la $t4,label_237_v88
lw $t4,0($t4)
slt $t3,$t5,$t4
la $t0,label_239_v89
sw $t3,0($t0)
# Original instruction: bnez v89,label_6_body
la $t5,label_239_v89
lw $t5,0($t5)
bnez $t5,label_6_body
label_4_next:
# Original instruction: addiu v90,$fp,-10008
addiu $t5,$fp,-10008
la $t0,label_243_v90
sw $t5,0($t0)
# Original instruction: li v91,0
li $t5,0
la $t0,label_242_v91
sw $t5,0($t0)
# Original instruction: sw v91,0(v90)
la $t5,label_242_v91
lw $t5,0($t5)
la $t4,label_243_v90
lw $t4,0($t4)
sw $t5,0($t4)
# Original instruction: addiu v92,$fp,-10008
addiu $t5,$fp,-10008
la $t0,label_245_v92
sw $t5,0($t0)
# Original instruction: lw v93,0(v92)
la $t5,label_245_v92
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_250_v93
sw $t4,0($t0)
# Original instruction: addiu v94,$fp,4
addiu $t5,$fp,4
la $t0,label_248_v94
sw $t5,0($t0)
# Original instruction: lw v95,0(v94)
la $t5,label_248_v94
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_251_v95
sw $t4,0($t0)
# Original instruction: slt v96,v93,v95
la $t5,label_250_v93
lw $t5,0($t5)
la $t4,label_251_v95
lw $t4,0($t4)
slt $t3,$t5,$t4
la $t0,label_253_v96
sw $t3,0($t0)
# Original instruction: beqz v96,label_14_next
la $t5,label_253_v96
lw $t5,0($t5)
beqz $t5,label_14_next
label_16_body:
# Original instruction: addiu v97,$fp,-10012
addiu $t5,$fp,-10012
la $t0,label_257_v97
sw $t5,0($t0)
# Original instruction: li v98,0
li $t5,0
la $t0,label_256_v98
sw $t5,0($t0)
# Original instruction: sw v98,0(v97)
la $t5,label_256_v98
lw $t5,0($t5)
la $t4,label_257_v97
lw $t4,0($t4)
sw $t5,0($t4)
# Original instruction: addiu v99,$fp,-10012
addiu $t5,$fp,-10012
la $t0,label_259_v99
sw $t5,0($t0)
# Original instruction: lw v100,0(v99)
la $t5,label_259_v99
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_265_v100
sw $t4,0($t0)
# Original instruction: addiu v101,$fp,-10008
addiu $t5,$fp,-10008
la $t0,label_262_v101
sw $t5,0($t0)
# Original instruction: lw v102,0(v101)
la $t5,label_262_v101
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_264_v102
sw $t4,0($t0)
# Original instruction: slt v103,v102,v100
la $t5,label_264_v102
lw $t5,0($t5)
la $t4,label_265_v100
lw $t4,0($t4)
slt $t3,$t5,$t4
la $t0,label_269_v103
sw $t3,0($t0)
# Original instruction: xori v103,v103,1
la $t4,label_269_v103
lw $t4,0($t4)
xori $t4,$t4,1
la $t0,label_269_v103
sw $t4,0($t0)
# Original instruction: beqz v103,label_17_next
la $t5,label_269_v103
lw $t5,0($t5)
beqz $t5,label_17_next
label_19_body:
# Original instruction: addiu v106,$fp,-10004
addiu $t5,$fp,-10004
la $t0,label_279_v106
sw $t5,0($t0)
# Original instruction: addiu v107,$fp,-10008
addiu $t5,$fp,-10008
la $t0,label_272_v107
sw $t5,0($t0)
# Original instruction: lw v108,0(v107)
la $t5,label_272_v107
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_275_v108
sw $t4,0($t0)
# Original instruction: li v109,200
li $t5,200
la $t0,label_276_v109
sw $t5,0($t0)
# Original instruction: mul v105,v108,v109
la $t5,label_275_v108
lw $t5,0($t5)
la $t4,label_276_v109
lw $t4,0($t4)
mul $t3,$t5,$t4
la $t0,label_289_v105
sw $t3,0($t0)
# Original instruction: add v105,v105,v106
la $t3,label_289_v105
lw $t3,0($t3)
la $t4,label_279_v106
lw $t4,0($t4)
add $t3,$t3,$t4
la $t0,label_289_v105
sw $t3,0($t0)
# Original instruction: addiu v110,$fp,-10012
addiu $t5,$fp,-10012
la $t0,label_282_v110
sw $t5,0($t0)
# Original instruction: lw v111,0(v110)
la $t5,label_282_v110
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_285_v111
sw $t4,0($t0)
# Original instruction: li v112,4
li $t5,4
la $t0,label_286_v112
sw $t5,0($t0)
# Original instruction: mul v104,v111,v112
la $t5,label_285_v111
lw $t5,0($t5)
la $t4,label_286_v112
lw $t4,0($t4)
mul $t3,$t5,$t4
la $t0,label_291_v104
sw $t3,0($t0)
# Original instruction: add v104,v104,v105
la $t3,label_291_v104
lw $t3,0($t3)
la $t4,label_289_v105
lw $t4,0($t4)
add $t3,$t3,$t4
la $t0,label_291_v104
sw $t3,0($t0)
# Original instruction: lw v113,0(v104)
la $t5,label_291_v104
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_294_v113
sw $t4,0($t0)
# Original instruction: addiu v114,$sp,-4
addiu $t5,$sp,-4
la $t0,label_295_v114
sw $t5,0($t0)
# Original instruction: sw v113,0(v114)
la $t5,label_294_v113
lw $t5,0($t5)
la $t4,label_295_v114
lw $t4,0($t4)
sw $t5,0($t4)
# Original instruction: addiu $sp,$sp,-4
addiu $sp,$sp,-4
# Original instruction: jal print_i
jal print_i
# Original instruction: addiu $sp,$sp,4
addiu $sp,$sp,4
# Original instruction: la v115,label_0_str
la $t5,label_0_str
la $t0,label_298_v115
sw $t5,0($t0)
# Original instruction: addiu v116,$sp,-4
addiu $t5,$sp,-4
la $t0,label_299_v116
sw $t5,0($t0)
# Original instruction: sw v115,0(v116)
la $t5,label_298_v115
lw $t5,0($t5)
la $t4,label_299_v116
lw $t4,0($t4)
sw $t5,0($t4)
# Original instruction: addiu $sp,$sp,-4
addiu $sp,$sp,-4
# Original instruction: jal print_s
jal print_s
# Original instruction: addiu $sp,$sp,4
addiu $sp,$sp,4
# Original instruction: addiu v117,$fp,-10012
addiu $t5,$fp,-10012
la $t0,label_309_v117
sw $t5,0($t0)
# Original instruction: addiu v118,$fp,-10012
addiu $t5,$fp,-10012
la $t0,label_302_v118
sw $t5,0($t0)
# Original instruction: lw v119,0(v118)
la $t5,label_302_v118
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_305_v119
sw $t4,0($t0)
# Original instruction: li v120,1
li $t5,1
la $t0,label_306_v120
sw $t5,0($t0)
# Original instruction: add v121,v119,v120
la $t5,label_305_v119
lw $t5,0($t5)
la $t4,label_306_v120
lw $t4,0($t4)
add $t3,$t5,$t4
la $t0,label_308_v121
sw $t3,0($t0)
# Original instruction: sw v121,0(v117)
la $t5,label_308_v121
lw $t5,0($t5)
la $t4,label_309_v117
lw $t4,0($t4)
sw $t5,0($t4)
label_18_posttest:
# Original instruction: addiu v122,$fp,-10012
addiu $t5,$fp,-10012
la $t0,label_311_v122
sw $t5,0($t0)
# Original instruction: lw v123,0(v122)
la $t5,label_311_v122
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_317_v123
sw $t4,0($t0)
# Original instruction: addiu v124,$fp,-10008
addiu $t5,$fp,-10008
la $t0,label_314_v124
sw $t5,0($t0)
# Original instruction: lw v125,0(v124)
la $t5,label_314_v124
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_316_v125
sw $t4,0($t0)
# Original instruction: slt v126,v125,v123
la $t5,label_316_v125
lw $t5,0($t5)
la $t4,label_317_v123
lw $t4,0($t4)
slt $t3,$t5,$t4
la $t0,label_321_v126
sw $t3,0($t0)
# Original instruction: xori v126,v126,1
la $t4,label_321_v126
lw $t4,0($t4)
xori $t4,$t4,1
la $t0,label_321_v126
sw $t4,0($t0)
# Original instruction: bnez v126,label_19_body
la $t5,label_321_v126
lw $t5,0($t5)
bnez $t5,label_19_body
label_17_next:
# Original instruction: la v127,label_1_str
la $t5,label_1_str
la $t0,label_324_v127
sw $t5,0($t0)
# Original instruction: addiu v128,$sp,-4
addiu $t5,$sp,-4
la $t0,label_325_v128
sw $t5,0($t0)
# Original instruction: sw v127,0(v128)
la $t5,label_324_v127
lw $t5,0($t5)
la $t4,label_325_v128
lw $t4,0($t4)
sw $t5,0($t4)
# Original instruction: addiu $sp,$sp,-4
addiu $sp,$sp,-4
# Original instruction: jal print_s
jal print_s
# Original instruction: addiu $sp,$sp,4
addiu $sp,$sp,4
# Original instruction: addiu v129,$fp,-10008
addiu $t5,$fp,-10008
la $t0,label_335_v129
sw $t5,0($t0)
# Original instruction: addiu v130,$fp,-10008
addiu $t5,$fp,-10008
la $t0,label_328_v130
sw $t5,0($t0)
# Original instruction: lw v131,0(v130)
la $t5,label_328_v130
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_331_v131
sw $t4,0($t0)
# Original instruction: li v132,1
li $t5,1
la $t0,label_332_v132
sw $t5,0($t0)
# Original instruction: add v133,v131,v132
la $t5,label_331_v131
lw $t5,0($t5)
la $t4,label_332_v132
lw $t4,0($t4)
add $t3,$t5,$t4
la $t0,label_334_v133
sw $t3,0($t0)
# Original instruction: sw v133,0(v129)
la $t5,label_334_v133
lw $t5,0($t5)
la $t4,label_335_v129
lw $t4,0($t4)
sw $t5,0($t4)
label_15_posttest:
# Original instruction: addiu v134,$fp,-10008
addiu $t5,$fp,-10008
la $t0,label_337_v134
sw $t5,0($t0)
# Original instruction: lw v135,0(v134)
la $t5,label_337_v134
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_342_v135
sw $t4,0($t0)
# Original instruction: addiu v136,$fp,4
addiu $t5,$fp,4
la $t0,label_340_v136
sw $t5,0($t0)
# Original instruction: lw v137,0(v136)
la $t5,label_340_v136
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_343_v137
sw $t4,0($t0)
# Original instruction: slt v138,v135,v137
la $t5,label_342_v135
lw $t5,0($t5)
la $t4,label_343_v137
lw $t4,0($t4)
slt $t3,$t5,$t4
la $t0,label_345_v138
sw $t3,0($t0)
# Original instruction: bnez v138,label_16_body
la $t5,label_345_v138
lw $t5,0($t5)
bnez $t5,label_16_body
label_14_next:
print_pascals_triangle_epilogue:
# Original instruction: popRegisters
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_276_v109
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_316_v125
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_113_v24
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_234_v87
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_76_v19
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_248_v94
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_133_v35
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_54_v9
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_86_v23
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_129_v41
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_81_v20
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_151_v51
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_343_v137
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_209_v75
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_334_v133
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_342_v135
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_207_v70
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_59_v12
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_179_v59
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_126_v40
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_95_v28
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_219_v79
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_305_v119
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_212_v77
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_82_v22
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_51_v8
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_291_v104
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_226_v83
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_162_v44
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_175_v63
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_272_v107
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_340_v136
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_259_v99
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_60_v10
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_237_v88
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_314_v124
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_324_v127
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_37_v1
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_64_v13
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_309_v117
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_231_v85
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_328_v130
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_282_v110
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_171_v61
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_321_v126
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_306_v120
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_45_v3
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_298_v115
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_48_v6
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_228_v84
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_239_v89
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_332_v132
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_140_v47
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_331_v131
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_99_v26
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_102_v30
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_43_v4
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_222_v81
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_194_v68
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_325_v128
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_74_v18
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_119_v38
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_236_v86
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_120_v39
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_285_v111
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_264_v102
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_105_v31
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_269_v103
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_335_v129
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_158_v54
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_89_v14
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_40_v2
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_164_v43
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_242_v91
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_70_v17
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_257_v97
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_279_v106
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_275_v108
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_52_v7
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_299_v116
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_253_v96
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_168_v60
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_79_v21
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_251_v95
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_130_v42
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_69_v16
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_250_v93
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_46_v5
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_265_v100
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_204_v73
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_286_v112
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_96_v29
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_185_v66
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_302_v118
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_289_v105
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_308_v121
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_197_v34
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_159_v55
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_243_v90
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_189_v58
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_196_v69
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_137_v46
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_66_v15
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_172_v62
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_186_v67
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_106_v32
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_38_v0
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_154_v52
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_229_v80
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_109_v25
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_345_v138
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_176_v64
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_116_v37
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_193_v56
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_57_v11
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_92_v27
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_225_v82
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_214_v78
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_337_v134
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_206_v74
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_144_v49
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_145_v50
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_317_v123
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_294_v113
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_256_v98
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_245_v92
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_141_v48
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_200_v71
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_182_v65
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_215_v76
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_191_v57
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_148_v45
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_295_v114
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_155_v53
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_203_v72
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_123_v36
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_262_v101
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_311_v122
sw $t0,0($t1)
lw $t0,0($sp)
addiu $sp,$sp,4
la $t1,label_112_v33
sw $t0,0($t1)
# Original instruction: lw $ra,-4($fp)
lw $ra,-4($fp)
# Original instruction: addiu $sp,$fp,4
addiu $sp,$fp,4
# Original instruction: lw $fp,0($fp)
lw $fp,0($fp)
# Original instruction: jr $ra
jr $ra

.text
.globl main
main:
# Original instruction: addiu $fp,$sp,0
addiu $fp,$sp,0
# Original instruction: addiu $sp,$sp,-4
addiu $sp,$sp,-4
# Original instruction: la v139,label_2_str
la $t5,label_2_str
la $t0,label_348_v139
sw $t5,0($t0)
# Original instruction: addiu v140,$sp,-4
addiu $t5,$sp,-4
la $t0,label_349_v140
sw $t5,0($t0)
# Original instruction: sw v139,0(v140)
la $t5,label_348_v139
lw $t5,0($t5)
la $t4,label_349_v140
lw $t4,0($t4)
sw $t5,0($t4)
# Original instruction: addiu $sp,$sp,-4
addiu $sp,$sp,-4
# Original instruction: jal print_s
jal print_s
# Original instruction: addiu $sp,$sp,4
addiu $sp,$sp,4
# Original instruction: addiu v141,$fp,-4
addiu $t5,$fp,-4
la $t0,label_355_v141
sw $t5,0($t0)
# Original instruction: addiu $sp,$sp,-4
addiu $sp,$sp,-4
# Original instruction: jal read_i
jal read_i
# Original instruction: addi v142,$sp,0
addi $t5,$sp,0
la $t0,label_352_v142
sw $t5,0($t0)
# Original instruction: lw v143,0(v142)
la $t5,label_352_v142
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_354_v143
sw $t4,0($t0)
# Original instruction: addiu $sp,$sp,4
addiu $sp,$sp,4
# Original instruction: sw v143,0(v141)
la $t5,label_354_v143
lw $t5,0($t5)
la $t4,label_355_v141
lw $t4,0($t4)
sw $t5,0($t4)
# Original instruction: addiu v144,$fp,-4
addiu $t5,$fp,-4
la $t0,label_357_v144
sw $t5,0($t0)
# Original instruction: lw v145,0(v144)
la $t5,label_357_v144
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_360_v145
sw $t4,0($t0)
# Original instruction: addiu v146,$sp,-4
addiu $t5,$sp,-4
la $t0,label_361_v146
sw $t5,0($t0)
# Original instruction: sw v145,0(v146)
la $t5,label_360_v145
lw $t5,0($t5)
la $t4,label_361_v146
lw $t4,0($t4)
sw $t5,0($t4)
# Original instruction: addiu $sp,$sp,-4
addiu $sp,$sp,-4
# Original instruction: jal print_pascals_triangle
jal print_pascals_triangle
# Original instruction: addiu $sp,$sp,4
addiu $sp,$sp,4
# Original instruction: j main_epilogue
j main_epilogue
main_epilogue:
# Original instruction: li $v0,10
li $v0,10
# Original instruction: syscall
syscall

.text
print_s:
# Original instruction: addiu $sp,$sp,-4
addiu $sp,$sp,-4
# Original instruction: sw $fp,0($sp)
sw $fp,0($sp)
# Original instruction: addi $fp,$sp,0
addi $fp,$sp,0
# Original instruction: addiu $sp,$sp,0
addiu $sp,$sp,0
# Original instruction: pushRegisters
# Original instruction: lw $a0,4($fp)
lw $a0,4($fp)
# Original instruction: li $v0,4
li $v0,4
# Original instruction: syscall
syscall
print_s_epilogue:
# Original instruction: popRegisters
# Original instruction: addiu $sp,$fp,4
addiu $sp,$fp,4
# Original instruction: lw $fp,0($fp)
lw $fp,0($fp)
# Original instruction: jr $ra
jr $ra

.text
print_i:
# Original instruction: addiu $sp,$sp,-4
addiu $sp,$sp,-4
# Original instruction: sw $fp,0($sp)
sw $fp,0($sp)
# Original instruction: addi $fp,$sp,0
addi $fp,$sp,0
# Original instruction: addiu $sp,$sp,0
addiu $sp,$sp,0
# Original instruction: pushRegisters
# Original instruction: lw $a0,4($fp)
lw $a0,4($fp)
# Original instruction: li $v0,1
li $v0,1
# Original instruction: syscall
syscall
print_i_epilogue:
# Original instruction: popRegisters
# Original instruction: addiu $sp,$fp,4
addiu $sp,$fp,4
# Original instruction: lw $fp,0($fp)
lw $fp,0($fp)
# Original instruction: jr $ra
jr $ra

.text
print_c:
# Original instruction: addiu $sp,$sp,-4
addiu $sp,$sp,-4
# Original instruction: sw $fp,0($sp)
sw $fp,0($sp)
# Original instruction: addi $fp,$sp,0
addi $fp,$sp,0
# Original instruction: addiu $sp,$sp,0
addiu $sp,$sp,0
# Original instruction: pushRegisters
# Original instruction: lw $a0,4($fp)
lw $a0,4($fp)
# Original instruction: li $v0,11
li $v0,11
# Original instruction: syscall
syscall
print_c_epilogue:
# Original instruction: popRegisters
# Original instruction: addiu $sp,$fp,4
addiu $sp,$fp,4
# Original instruction: lw $fp,0($fp)
lw $fp,0($fp)
# Original instruction: jr $ra
jr $ra

.text
read_c:
# Original instruction: addiu $sp,$sp,-4
addiu $sp,$sp,-4
# Original instruction: sw $fp,0($sp)
sw $fp,0($sp)
# Original instruction: addi $fp,$sp,0
addi $fp,$sp,0
# Original instruction: addiu $sp,$sp,0
addiu $sp,$sp,0
# Original instruction: pushRegisters
# Original instruction: li $v0,12
li $v0,12
# Original instruction: syscall
syscall
# Original instruction: sw $v0,4($fp)
sw $v0,4($fp)
read_c_epilogue:
# Original instruction: popRegisters
# Original instruction: addiu $sp,$fp,4
addiu $sp,$fp,4
# Original instruction: lw $fp,0($fp)
lw $fp,0($fp)
# Original instruction: jr $ra
jr $ra

.text
read_i:
# Original instruction: addiu $sp,$sp,-4
addiu $sp,$sp,-4
# Original instruction: sw $fp,0($sp)
sw $fp,0($sp)
# Original instruction: addi $fp,$sp,0
addi $fp,$sp,0
# Original instruction: addiu $sp,$sp,0
addiu $sp,$sp,0
# Original instruction: pushRegisters
# Original instruction: li $v0,5
li $v0,5
# Original instruction: syscall
syscall
# Original instruction: sw $v0,4($fp)
sw $v0,4($fp)
read_i_epilogue:
# Original instruction: popRegisters
# Original instruction: addiu $sp,$fp,4
addiu $sp,$fp,4
# Original instruction: lw $fp,0($fp)
lw $fp,0($fp)
# Original instruction: jr $ra
jr $ra

.text
mcmalloc:
# Original instruction: addiu $sp,$sp,-4
addiu $sp,$sp,-4
# Original instruction: sw $fp,0($sp)
sw $fp,0($sp)
# Original instruction: addi $fp,$sp,0
addi $fp,$sp,0
# Original instruction: addiu $sp,$sp,0
addiu $sp,$sp,0
# Original instruction: pushRegisters
# Original instruction: lw $a0,8($fp)
lw $a0,8($fp)
# Original instruction: li $v0,9
li $v0,9
# Original instruction: syscall
syscall
# Original instruction: sw $v0,4($fp)
sw $v0,4($fp)
mcmalloc_epilogue:
# Original instruction: popRegisters
# Original instruction: addiu $sp,$fp,4
addiu $sp,$fp,4
# Original instruction: lw $fp,0($fp)
lw $fp,0($fp)
# Original instruction: jr $ra
jr $ra

